...
// - Self-contained: injects its own CSS & DOM
// - Firestore single-doc model: rooms/{roomId}/volatile/chat
// - Auto-mounts after Net is ready & a room is joined (listens for "net-room")
...
const BOTTOM_OFFSET_PX = 48;           // float above map footer; tweak to taste

// --- Inject minimal CSS (amber HUD look + rank fades) ---
(function injectCSS(){
  if (document.getElementById('mss-chat-styles')) return;
  const css = `
  /* ===== MSS:84 HUD Chat (self-contained) ===== */
  #mss-chat-wrap {
    position: absolute;
    left: 16px; right: 16px;
    bottom: ${BOTTOM_OFFSET_PX}px;
    z-index: 777; /* above map UI */
  ...
  .mss-chat-lines {
    ...
    background: rgba(16,16,16,0.35);  /* translucent panel */
    box-shadow: 0 0 0 1px rgba(255, 191, 0, 0.08) inset, 0 10px 24px rgba(0,0,0,0.35);
    pointer-events: auto;             /* let user select/copy text if desired */
  }
  .mss-chat-line {
    position: relative;
  ...
  /* Per-rank background strength (newest strongest, oldest ~none) */
  ...
  /* New message subtle pulse */
  .mss-chat-line.pulse { filter: drop-shadow(0 0 4px rgba(255,191,0,0.5)); }
  .mss-chat-line.pulse { animation: mssPulse 300ms ease; }
  @keyframes mssPulse {
    from { filter: drop-shadow(0 0 0 rgba(255,191,0,0)); }
    to   { filter: drop-shadow(0 0 6px rgba(255,191,0,0.55)); }
  }

  /* Input row */
  .mss-chat-input {
    display: grid; grid-template-columns: 1fr auto; gap: 6px; align-items: center;
    pointer-events: auto; /* focusable */
  }
...
const Chat = {
  ...
  // Build and attach UI inside .stage (or body fallback)
  _ensureUI() {
    ...
    wrap.innerHTML = `
      <div class="mss-chat" role="region" aria-label="Chat">
  ...
  _render() {
    ...
    if (!$lines) return;
    // newest at bottom; ranks computed bottom-up
    const view = [...this._cache].slice(-MAX);
    const html = view
      .map((m, idx) => {
        const rank = (view.length - 1) - idx; // 0=newest(bottom), 4=oldest(top)
        const who  = (m.user || '—');
  ...
  async _subscribe() {
    ...
    // chat doc (create-if-missing once)
    this._ref = doc(this._db, 'rooms', this._roomId, 'volatile', 'chat');
    try {
      // Ensure doc exists but don't overwrite
      await setDoc(this._ref, { messages: [], updatedAt: serverTimestamp() }, { merge: true });
    } catch {}
  ...
  async send(inputElOrString) {
    ...
    // Transaction: append + trim atomically to last MAX
...
(function autoMount(){
  // If Net already has a room id, mount immediately; else wait for event.
  const tryStart = () => {
    const roomId = window.Net?.roomId; // requires tiny getter (see edit below)
    if (roomId) {
  ...
  if (!tryStart()) {
    // Wait for a room join event emitted by network.js (see edit below)

...
// --- Leaf pattern (dense, scales with `u`) ---
pat('pat-leaf', u, u * 0.5, (p) => {
  // scale original 80×40 geometry into your u × (u*0.5) tile
  const g = document.createElementNS(svgNS, 'g');
  g.setAttribute('transform', `scale(${u / 80} ${(u * 0.5) / 40})`);

  const path = document.createElementNS(svgNS, 'path');
  // original leaf paths (from the 80×40 tile)
  path.setAttribute(
    'd',

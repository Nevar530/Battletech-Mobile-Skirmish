...
(function mountGatoButton(){
  const mechMenu = document.querySelector('[data-panel="mech"] .panel__head');
  if (!mechMenu) return;
  const btn = document.createElement('button');
  btn.textContent = 'GATO';
  btn.addEventListener('click', toggleGatoDrawer);
  mechMenu.appendChild(btn);
})();

let gatoDrawer = null;

function toggleGatoDrawer(){
  if (!gatoDrawer){
    gatoDrawer = document.createElement('aside');
    gatoDrawer.className = 'gato-drawer';
    gatoDrawer.innerHTML = `
      <header><strong>GATO Rolls</strong><button id="gato-close">✕</button></header>
      <div>
        <label>Range <input type="number" id="gato-range" value="5"></label>
        <label>Light woods <input type="number" id="gato-woods-light" value="0"></label>
        <label>Heavy woods <input type="number" id="gato-woods-heavy" value="0"></label>
        <label><input type="checkbox" id="gato-partial"> Partial cover</label>
        <label><input type="checkbox" id="gato-called"> Called shot</label>
        <div>
          <button id="gato-calc">Compute TN</button>
          <button id="gato-roll">Roll Attack</button>
        </div>
        <pre id="gato-out">Ready.</pre>
      </div>
    `;
    document.body.appendChild(gatoDrawer);
    document.getElementById('gato-close').onclick = ()=> gatoDrawer.classList.remove('open');
    document.getElementById('gato-calc').onclick = handleCompute;
    document.getElementById('gato-roll').onclick = handleRoll;
  }
  gatoDrawer.classList.toggle('open');
}

function readCtx(){
  return {
    range: +document.getElementById('gato-range').value,
    lightWoodsHexes: +document.getElementById('gato-woods-light').value,
    heavyWoodsHexes: +document.getElementById('gato-woods-heavy').value,
    calledShot: document.getElementById('gato-called').checked,
    los: document.getElementById('gato-partial').checked ? 'partial' : 'clear',
    modifiers:{ custom:0 },
  };
}
...
function handleCompute(){
  const ctx = readCtx();
  const tn = GATO.computeToHit(
    { gunnery:4 },      // player just knows their gunnery
    { movedHexes:0 },   // simple defaults
    GATO.Weapons.ER_Medium,
    ctx
  );
  document.getElementById('gato-out').textContent = `TN to hit: ${tn}`;
}

function handleRoll(){
  const ctx = readCtx();
  const res = GATO.resolveAttack(
    { gunnery:4 }, 
    { movedHexes:0, hullFacing:'front' },
    GATO.Weapons.ER_Medium,
    ctx,
    GATO.RNG
  );
  document.getElementById('gato-out').textContent = JSON.stringify(res, null, 2);
}
